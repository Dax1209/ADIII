(() => {
  ...
  function updateProgress() {
    ...
    progressText.textContent = `${String(current + 1).padStart(2, '0')} / ${String(total).padStart(2, '0')}`;
  }
  ...
  function setRomanceParticles(enabled) {
    ...
    makeRomanceParticle();
    romanceParticleTimer = setInterval(makeRomanceParticle, 2600);
  }
  ...
  function fillDiaryDust() {
    const dust = document.getElementById('diaryDust');
    if (!dust || dust.children.length) return;
    const amount = reducedMotion ? 12 : 34;
    for (let i = 0; i < amount; i++) {
      const mote = document.createElement('i');
      mote.style.setProperty('--dust-x', `${Math.random() * 100}%`);
      mote.style.setProperty('--dust-y', `${Math.random() * 100}%`);
      mote.style.setProperty('--dust-size', `${1 + Math.random() * 4}px`);
      mote.style.setProperty('--dust-alpha', `${.08 + Math.random() * .2}`);
      mote.style.setProperty('--dust-delay', `${Math.random() * -18}s`);
      mote.style.setProperty('--dust-time', `${14 + Math.random() * 16}s`);
  ...
  themeToggle.addEventListener('click', () => {
    const opening = moodPalette.hidden;
    moodPalette.hidden = !opening;
    themeToggle.setAttribute('aria-expanded', String(opening));
    if (opening) moodPalette.querySelector(`[data-theme-choice="${theme}"]`)?.focus({preventScroll:true});
  });
  moodPalette.addEventListener('click', event => {
    const choice = event.target.closest('[data-theme-choice]');
    if (!choice) return;
    applyTheme(choice.dataset.themeChoice);
    moodPalette.hidden = true;
    themeToggle.setAttribute('aria-expanded', 'false');
    themeToggle.focus({preventScroll:true});
  });
  document.addEventListener('pointerdown', event => {
    if (!event.target.closest('.mood-picker') && !moodPalette.hidden) {
      moodPalette.hidden = true;
      themeToggle.setAttribute('aria-expanded', 'false');
    }
  });
